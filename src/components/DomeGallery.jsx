...
function buildItems(pool, seg) {
    ...
    const normalizedImages = pool.map(item => {
        if (typeof item === 'string') {
            return { src: item, alt: '', thumbnail: null };
        }
        ...
        // Derive thumbnail path: <basename>.thumb.webp (generated by scripts/generate_thumbnails.js)
        // If the item already has an explicit thumbnail, use it; otherwise derive from src
        let thumbnail = item.thumbnail || null;
        if (!thumbnail && src && !src.startsWith('https://ui-avatars')) {
            const ext = src.lastIndexOf('.');
            if (ext !== -1) {
                thumbnail = src.slice(0, ext) + '.thumb.webp';
            }
        }
                            ...
                            >
                                ...
                                >
                                    <img
                                        ...
                                        decoding="async"
                                        onError={(e) => {
                                            // Fallback to full-res if thumbnail doesn't exist yet
                                            if (e.target.src !== it.src) {
                                                e.target.src = it.src;
                                            }
                                        }}
                                    />
